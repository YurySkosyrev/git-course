...
Иногда нужно сделать полноценное слияние даже в том случае, когда можно обойтись простой перемоткой.

При этом могут возникнуть проблемы:
* получаем ровную историю коммитов, из которой не ясно где были коммиты на основной ветке, а где на ветке разработки
* в новом функционале могут быть найдены ошибки и не понятно как отменить слияние с основной веткой. Если слияиние сделано только что, то можно сделать **git reset --hard ORIG_HEAD**. Если прошло много времени, то нужно искать в log или reflog.

**git merge --no-ff branch** - несмотря на то, что возможен коммит перемоткой, он будет осуществлен слиянием.

**git config merge.ff false** - установить в конфиге запрет на слияние перемоткой.

**git config branch.master.mergeoptioptions '--no-ff'** - список опций, который применяет для слияния в конкретную ветку.

**git merge --ff feature** - если перемотка всё же нужна.

## 7.6 Git – Слияние – Создание коммита из ветки: merge --squash

Как правило история разработки важна, но бывают случаи, когда мы хотим интегрировать только результат без истории.

**git merge --squash branch** - изменения из ветки branch проиндексированы и после коммита на основной ветке будут добавлены как новый коммит без второго родителя. Это используется в случае небольших веток, которые сами по себе ценности не представляют или могут быть заменены одним коммитом.

Если мы производим слияиние при котором образуется конфликт, то мы попадаем в особое состояние, только в отличии от простого слияния не создается файл **.git/MERGE_HEAD**. При добавлении изменения в индекс и коммите создается простой коммит без второго родителя, так как ссылка на второго родителя берётся как раз из файла **MERGE_HEAD**. Поэтому не работают некоторые команды(--abort, --continue).

Вместо --abort можно сделать **git reset --merge, git reset --hard**.

Если просто набрать **git commit** возникнет состояние конфликта.

Вообще состояние конфликта возникает тогда, когда в индексе есть три версии одного файла.

## 8.1 Git – Копирование коммитов – Копирование коммитов: cherry-pick

Команда **cherry-pick** позволяет копировать коммиты из одной ветки в другую. Обычно это нужно в случае обнаружения ошибки в файле, который есть одновременно в двух или более ветках. Ошибка исправляется, делается коммит и он копируется на текущую ветку. При этом на текущей ветке создаётся новый коммит (оба этих коммита называются эквивалентными и на текущей ветке обозначается штрихом D').

В отличии от слияния **cherry-pick** скопирует только указанный коммит.

**git cherry-pick D** - скопировать коммит D, при этом будет создан новый коммит D'.

Дублировать коммиты не очень хорошая практика. Если ошибка была найдена до разветвления, то лучше создать новую ветку из этого коммита, исправить ошибку и смержить новую ветку на текущие.

Но часто бывает, что кто-то уже исправил ошибку в своей ветке и это воспринимается как факт, который нужно учесть на других ветках.

**git cherry-pick -x D** - добавить к описанию строку, откуда был скопирован коммит.

**git cherry-pick master..feature** - можно копировать несколько коммитов, например в данном случае скопируются коммиты **feature**, которых нет в **master**.

Если при копировании коммитов возникает конфликт, то **cherry-pick** переходит в особое состояние при котором возможны 3 варианта:<br>
* **git cherry-pick --abort** - отменит все изменения и вернет всё до начала **cherry-pick**
* **git cherry-pick --continue** - продолжит выполнение **cherry-pick**, невозможно пока конфликты не будут разрешены.
* **git cherry-pick --quit** - остановится там, где мы сейчас и сбросить запомненное состояние. Т.е. успешно скопированные коммиты остаются, выполнение операции **cherry-pick** прекращается.

**git reset --hard @~2** - если нужно отменить **cherry-pick** (сдесь было скопировано 2 коммита). В reflog можно посмотреть историю коммитов и сколько их было скопировано при **cherry-pick**

**git cherry-pick --no-commit** - изменения вносятся только в рабочую директорию и индекс без коммита. Можно с флагом **-n**.
